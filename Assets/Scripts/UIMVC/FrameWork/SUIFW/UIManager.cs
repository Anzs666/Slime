...
namespace SUIFW
{
    public class UIManager : MonoBehaviour
    {
        ...
        /// </summary>
        private void SetUIFormPreShow(BaseUIForm baseUIForms, string uiFormName)
        {
	    ...
	    private void LoadFormsToAllUIFormsCatch(string uiFormsName)
        {
            ...
            {
                //是否清空“栈集合”中得数据
                if (baseUIResult.CurrentUIType.IsClearStack)
                {
                    ClearStackArray();
                }
                SetUIFormPreShow(baseUIResult, uiFormsName);
            }
	    ...
	    private void LoadUIForm(string uiFormName)
        {
            ...
            ////根据UI窗体名称，得到对应的ab加载路径
            _DicFormsPaths.TryGetValue(uiFormName, out strUIFormPaths);
            //根据“UI窗体名称”，加载“预设克隆体”////改为使用AB包加载
            if (!string.IsNullOrEmpty(strUIFormPaths))
            {
                //goCloneUIPrefabs = ResourcesMgr.GetInstance().LoadAsset(strUIFormPaths, false);
                string[] pathsArray = strUIFormPaths.ToLower().Split('\\');
                UnityEngine.Object tmpObj = null;
                StartCoroutine(AssetBundleMgr.Instance.LoadAssetBundlePack(pathsArray[0], strUIFormPaths+".ab",
                    (string abName) =>
                    {
                        tmpObj = (UnityEngine.Object)AssetBundleMgr.Instance.LoadAsset(pathsArray[0], abName, uiFormName, false);
                        goCloneUIPrefabs = Instantiate(tmpObj) as GameObject;
                        ABCompleteUIFormLoad(goCloneUIPrefabs);
                    }));
            }
        ...
        /// <summary>
        /// AssetBundle加载完成加入窗体集合，并设置根节点入栈
        /// </summary>
        /// <param name="goCloneUIPrefabs"></param>
        private void ABCompleteUIFormLoad(GameObject goCloneUIPrefabs)
        {
            ...
            {
                Debug.Log("_TraCanvasTransfrom==null Or goCloneUIPrefabs==null!! ,Plese Check!, 参数uiFormName=" + goCloneUIPrefabs.name);
            }
            //Debug.Log("出现不可以预估的错误，请检查，参数 uiFormName=" + goCloneUIPrefabs.name);
            //return null;
        }
